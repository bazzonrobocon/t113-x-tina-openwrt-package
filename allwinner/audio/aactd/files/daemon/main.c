...
int main(int argc, char *argv[])
{
    ...
    int opt;
    while ((opt = getopt_long(argc, argv, "hb:p:v:", opts, NULL)) != -1) {
        switch (opt) {
        case 'h':
            help_msg();
            ret = 0;
    ...
    if (ret < 0) {
        aactd_error("Failed to bind address %s and port %d to socket (fd: %d) (%s)\n",
                inet_ntop(AF_INET, &server_addr.sin_addr, inet_addr_str, INET_ADDR_STR_LEN),
                ntohs(server_addr.sin_port), listen_fd, strerror(errno));
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (verbose_level >= 2) {
                aactd_print_original_buf(remote_com_buf, read_bytes);
            }
            ...
            AACTD_DEBUG(0, "Read data (%zd bytes)\n", read_bytes);
            if (verbose_level >= 2) {
                aactd_print_original_buf(remote_com.data, read_bytes);
            }
