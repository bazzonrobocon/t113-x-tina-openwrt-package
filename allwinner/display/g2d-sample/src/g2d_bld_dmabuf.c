...
int parse_cmdline(int argc, char **argv, struct test_info_t *p)
{
	...
	while(i < argc) {
		...
		if (!strcmp(argv[i], "-src_resize")) {
			if (argc > i + 2) {
				i++;
				p->info.src_image[0].resize.w = atoi(argv[i]);
				i++;
				p->info.src_image[0].resize.h = atoi(argv[i]);
			} else {
				printf("-src_resize para err!\n\n");
				err ++;
			}
		}
...
int main(int argc, char **argv)
{
	...
	test_info.info.src_image[0].bbuff = 1;
	test_info.info.src_image[1].bbuff = 1;
	test_info.info.dst_image.bbuff = 1;
...
out2:
	...
	ion_memory_release(&test_info.src_ion2, src_fd2);
out1:
	munmap(test_info.src_ion.virt_addr, in_size);
