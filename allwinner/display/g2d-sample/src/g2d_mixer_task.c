...
unsigned int out_width[FRAME_TO_BE_PROCESS] = {
	192,  156,  108,  320,  447,  960, 241, 320,
...
FREE_DST:
	for (i = 0; i < FRAME_TO_BE_PROCESS; ++i) {
		ion_memory_release(&test_info.dst_ion[i], test_info.info[i].dst_image_h.fd);
		if (test_info.dst_fp[i])
			fclose(test_info.dst_fp[i]);
	}
FREE_SRC:
	for (i = 0; i < FRAME_TO_BE_PROCESS; ++i) {
		ion_memory_release(&test_info.src_ion[i], test_info.info[i].src_image_h.fd);
	}
OUT:
