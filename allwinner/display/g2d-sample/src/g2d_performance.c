...
int ion_memory_request(struct ion_info *ion, int mem_size)
{
#if USE_DMABUFHEAP
	int fd = -1, ret = 0;
	ion->bufferAllocator = CreateDmabufHeapBufferAllocator();
	if (!ion->bufferAllocator) {
		printf("unable to get allocator\n");
		return -1;
	}
	ret = MapDmabufHeapNameToIonHeap(ion->bufferAllocator, kDmabufSystemHeapName,
	...
	fd = DmabufHeapAllocSystem(ion->bufferAllocator, false /* cpu_access */, mem_size, 0, 0);
	if (fd < 0) {
		printf("DmabufHeapAllocSystem() failed: %d cpu_access: false\n", fd);
		return -1;
	}

	ion->virt_addr = mmap(NULL, mem_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
...
void ion_memory_release(struct ion_info *ion, int fd)
{
	close(fd);
#if USE_DMABUFHEAP
	FreeDmabufHeapBufferAllocator(ion->bufferAllocator);
#endif
	return;
}
...
static void show_fps(void) {
	...
	if (new.tv_sec * 1000 - old.tv_sec * 1000 >= 1000) {
		...
		if (fps_cnt > 0)
			printf("g2d_performance fps_cnt=%u cur_fps=%u, max_fps=%u, "
					"min_fps=%u, avg_fps=%.2f\n", fps_cnt, cur_fps, max_fps,
					min_fps, (float) avg_fps / (float) fps_cnt);
		first++;
		old = new;
		cur_fps = 0;
	} else {
...
int main(int argc, char **argv)
{
	...
	in_size = test_info.info.blt.src_image_h.width * test_info.info.blt.src_image_h.height * 3 / 2;
	out_size = test_info.info.blt.dst_image_h.width * test_info.info.blt.dst_image_h.height * 3 / 2;
	...
	printf("ready to open vs file %s \n", test_info.filename[2]);
	vs_fd = ion_memory_request(&test_info.vs_ion, out_size);
	printf("+++vs_fd = %d\n", vs_fd);
	if (vs_fd <= 0) {
		printf("ion_memory_request vs fd failed!\n");
		goto out2;
	}

	if((test_info.fp[2] = fopen(test_info.filename[2], "rt")) == NULL) {
		printf("open file %s fail. \n", test_info.filename[2]);
		ret = -1;
		goto out3;
	} else
		printf("open file %s ok. \n", test_info.filename[2]);
	ret = fread(test_info.vs_ion.virt_addr, out_size, 1, test_info.fp[2]);
	printf("read file %s ,ret = %d \n", test_info.filename[2], ret);

	test_info.info.blt.src_image_h.fd = src_fd;
	test_info.info.blt.dst_image_h.fd = dst_fd;

	test_info.info.blt.src_image_h.bbuff = 1;
	test_info.info.blt.dst_image_h.bbuff = 1;
	...
	while (1) {
		if (test_info.is_lbc) {
			if (ioctl(test_info.g2d_fd, G2D_CMD_LBC_ROT,
					(unsigned long) (&test_info.info)) < 0) {
			...
			if (ioctl(test_info.g2d_fd, G2D_CMD_BITBLT_H,
					(unsigned long) (&test_info.info.blt)) < 0) {
		...
		if (test_info.is_compare) {
			ret = memcmp(test_info.vs_ion.virt_addr,
					test_info.dst_ion.virt_addr, out_size);

			if (ret > 0) {
				printf("vs_ion is less than dst_ion\n");
				break;
			} else if (ret < 0) {
				printf("dst_ion is less than vs_ion\n");
				break;
			}
		}
...
out3:
	munmap(test_info.vs_ion.virt_addr, in_size);
	ion_memory_release(&test_info.vs_ion, vs_fd);
	fclose(test_info.fp[2]);

out2:
	munmap(test_info.dst_ion.virt_addr, out_size);
	ion_memory_release(&test_info.dst_ion, dst_fd);
	fclose(test_info.fp[1]);

out1:
	munmap(test_info.src_ion.virt_addr, in_size);
	ion_memory_release(&test_info.src_ion, src_fd);
