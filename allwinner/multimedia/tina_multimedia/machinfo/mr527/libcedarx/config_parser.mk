...
ifeq ($(CONFIG_CEDAR_PARSER_G729),y)
	CONFIG_PARSER += --enable-g729-parser=yes
else
	CONFIG_PARSER += --enable-g729-parser=no
endif

ifeq ($(CONFIG_CEDAR_PARSER_MMF),y)
	CONFIG_PARSER += --enable-mmf-parser=yes
else
	CONFIG_PARSER += --enable-mmf-parser=no
endif

ifeq ($(CONFIG_CEDAR_PARSER_DSD),y)
	CONFIG_PARSER += --enable-dsd-parser=yes
else
	CONFIG_PARSER += --enable-dsd-parser=no
endif

ifeq ($(CONFIG_CEDAR_PARSER_WV),y)
	CONFIG_PARSER += --enable-wv-parser=yes
else
	CONFIG_PARSER += --enable-wv-parser=no
endif

ifeq ($(CONFIG_CEDAR_PARSER_CAF),y)
	CONFIG_PARSER += --enable-caf-parser=yes
else
	CONFIG_PARSER += --enable-caf-parser=no
endif

ifeq ($(CONFIG_CEDAR_PARSER_MMS),y)
	CONFIG_PARSER += --enable-mms-parser=yes
else
	CONFIG_PARSER += --enable-mms-parser=no
endif
