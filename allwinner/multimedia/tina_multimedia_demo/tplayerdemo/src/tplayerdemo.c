...
#if TPLAYER_POWER_MONITOR
#include "power_monitor.h"

static int bPlaying = 0;
typedef struct DemoPMContext {
	tplayer_monitor	*demo_monitor;
	int				bPlaying;
	int				video_num;
	int				bSwitch;
	int				thrd_quit;
	pthread_t		process_thrd_id;
...
#if TPLAYERDEMO_SUSPEND_TEST
    {"start suspend",   COMMAND_START_SUSPEND_TEST,      "start the loop of suspend."},
    {"stop suspend",   COMMAND_STOP_SUSPEND_TEST,      "stop the loop of suspend."},
#endif
    {NULL, 0, NULL}
};
...
#if TPLAYER_POWER_MONITOR
/*TODO: start and destroy the thrd*/
static void *process_power_call(void *param)
{
	int ret = 0;
	DemoPMContext *p = (DemoPMContext*)param;
	while (1) {
		pthread_mutex_lock(&p->pm_lock);
		if (p->thrd_quit) {
			pthread_mutex_unlock(&p->pm_lock);
			break;
		}
		pthread_mutex_unlock(&p->pm_lock);
		if (!p->bSwitch) {
		...
		pthread_mutex_lock(&p->pm_lock);
		if (p->bPlaying == 1) {
			for (int i = 0; i < p->video_num; i++) {
				ret = TPlayerPause(gDemoPlayers[i].mTPlayer);
				if (ret < 0) {
					printf("[%s, %d]TPlayerPause[%d] failed\n", __func__,__LINE__, i);
					pthread_mutex_unlock(&p->pm_lock);
					break;
				}
			}
			if (!ret)
				p->bPlaying = 0;
			if (p->pause_ms > 0)
				usleep(p->pause_ms*1000);
			//sleep(1);
			tp_info("pause by powerkey, pause_ms = %d ms", p->pause_ms);
		} else if (p->bPlaying == 0) {
			for (int i = 0; i < p->video_num; i++) {
				ret = TPlayerStart(gDemoPlayers[i].mTPlayer);
				if (ret < 0) {
					printf("[%s, %d]TPlayerStart[%d] failed\n", __func__,__LINE__, i);
					pthread_mutex_unlock(&p->pm_lock);
					break;
				}
			}
			p->bPlaying = 1;
			if (p->start_ms > 0)
				usleep(p->start_ms*1000);
			tp_info("start by powerkey, start_ms = %d ms", p->start_ms);
		} else {
			printf("[%s, %d]error:p->bPlaying:%d is invalid status\n",__func__, __LINE__, p->bPlaying);
			ret = -1;
		}

		p->bSwitch = 0;
		send_done_2_pkm(p->demo_monitor);
		pthread_mutex_unlock(&p->pm_lock);
		if (!ret && p->bPlaying == 0)
...
int CallbackForPowerKeyScan(void *pUserData, int msg, int param0, void *param1)
{
	DemoPMContext *p = (DemoPMContext*)pUserData;
	int ret = 0;
	switch(msg)
	{
	case TPLAYER_MONITOR_CALL_POWERKEY:
	{
		printf("power key was pressed-msg\n");
		pthread_mutex_lock(&p->pm_lock);
		p->bSwitch = 1;
		pthread_mutex_unlock(&p->pm_lock);
		break;
...
static int createPlayersAndPlayVideos(int argc, char** argv)
{
    char fileType[FILE_TYPE_NUM][FILE_TYPE_LEN] = {".avi",".mkv",".flv",".ts",".mp4",".ts",".webm",".asf",".mpg",".mpeg",".mov",".vob",".3gp",".wmv",".pmp",".f4v",
                                                                                       ".mp1",".mp2",".mp3",".ogg",".flac",".ape",".wav",".m4a",".amr",".aac",".omg",".oma",".aa3",".caf",".opus"};
    char* lastStrPos;
    int ret = 0;
...
#if TPLAYER_POWER_MONITOR
		//* powerkey monitor only support multi-videos for now */
		 //* NOTE: argc >1 for testing, it should be 2, which means multi-videos playing
		DemoPMContext demo_pm_ctx;
		memset(&demo_pm_ctx, 0, sizeof(DemoPMContext));
		demo_pm_ctx.pause_ms = GetConfigParamterInt("tplayer_pause_ms", 100);
		demo_pm_ctx.start_ms = GetConfigParamterInt("tplayer_start_ms", 500); //* for making sure not get paused before done waking up.
		ret = pthread_mutex_init(&demo_pm_ctx.pm_lock, NULL);
		if (ret < 0) {
			printf("error:pm_lock create failed\n");
			goto QUIT;
		}
		ret = pthread_create(&demo_pm_ctx.process_thrd_id, NULL, process_power_call, &demo_pm_ctx);
		if (ret < 0) {
			printf("[%s %d]error:pm thread create failed\n", __FILE__, __LINE__);
			goto QUIT;
		}
		if (argc > 2) {
			demo_pm_ctx.demo_monitor = tplayer_start_monitor_powerkey(&demo_pm_ctx,CallbackForPowerKeyScan);
			if (!demo_pm_ctx.demo_monitor) {
				printf("error:tplayer - pm powerkey create failed");
				goto QUIT;
			}
			printf("demo_monitor create done\n");
			demo_pm_ctx.video_num = argc-1;
		}
		demo_pm_ctx.bPlaying = 1;
#endif
        while(!bQuit)
        {
            if(isDir)
            ...
            if(readCommand(strCommandLine, sizeof(strCommandLine)) == 0)
            {
                ...
                switch(nCommandId)
                {
                    ...
                    case COMMAND_QUIT:
                    {
                        ...
                        bQuit = 1;
#if TPLAYERDEMO_SUSPEND_TEST
                        tplayer_stop_suspend(gDemoPlayers, argc-1);
#endif
                        break;
                    }
#if TPLAYERDEMO_SUSPEND_TEST
                    case COMMAND_START_SUSPEND_TEST:
                    {
                        ret = tplayer_start_suspend(gDemoPlayers, argc-1);
                        if (ret != 0)
                            printf("[%s %d]start suspend failed\n", __FILE__, __LINE__);
                        break;
                    }
                    case COMMAND_STOP_SUSPEND_TEST:
                    {
                        ret = tplayer_stop_suspend(gDemoPlayers, argc-1);
                        if (ret != 0)
                            printf("[%s %d]stop suspend failed\n", __FILE__, __LINE__);
                        break;
                    }
#endif
                }
...
#if TPLAYER_POWER_MONITOR
        //* NOTE: argc >1 is for testing, it should be 2, which means multi-videos playing
        if (argc > 2) {
        pthread_mutex_destroy(&demo_pm_ctx.pm_lock);
        }
        if (argc > 2 && demo_pm_ctx.demo_monitor) {
            demo_pm_ctx.bPlaying = 0;
            demo_pm_ctx.thrd_quit = 1;
            int t_ret;
            if (demo_pm_ctx.process_thrd_id)
                pthread_join(demo_pm_ctx.process_thrd_id, &t_ret);
            tplayer_stop_powerkey(demo_pm_ctx.demo_monitor);
            demo_pm_ctx.demo_monitor = NULL;
        }
#endif
        //PowerManagerReleaseWakeLock("tplayerdemo");
        printf("\n");
