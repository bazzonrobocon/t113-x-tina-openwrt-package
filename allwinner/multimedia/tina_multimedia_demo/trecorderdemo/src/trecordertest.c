...
static const Command commands[] =
{
	...
	{"capture",	COMMAND_CAPTURE_PICTURE,		"capture picture of front or back camera, 0--front, 1--back."},
	{NULL, 0, NULL}
};
...
static int readCommand(char* strCommandLine, int nMaxLineSize)
{
	...
	if (result > 0) {
		if (FD_ISSET(STDIN_FILENO, &readFdSet)) {
			if(fgets(&strCommandLine[0], nMaxLineSize, stdin)){
				p = strCommandLine;
				while (*p != 0) {
					if (*p == 0xa) {
						*p = 0;
...
{
	printf("****************************************************************************\n");
	printf("* trecorderdemo 0: front channel recorder test\n");
	printf("* trecorderdemo 1: rear channel recorder test\n");
	printf("* trecorderdemo 2: front and rear two channel recorder test\n");
    printf("* trecorderdemo audio X: audio X recording test(X = 0/1)\n");
	printf("***************************************************************************\n");
}
...
int CallbackFromTRecorder(void* pUserData, int msg, void* param)
{
    ...
    if (!readKey(0, "mp4_output_path", path)) {
        printf("path:%s\n", path);
        strcpy(path, "/mnt/UDISK");
    }
	...
	switch(msg) {
		case T_RECORD_ONE_FILE_COMPLETE:
		{
            ...
            if(trTestContext->mRecorderFileCount > 3)
			...
			snprintf(trTestContext->mRecorderPath, sizeof(trTestContext->mRecorderPath),
		            "%s/AW_%s_video%d.mp4", path,num[trTestContext->mRecorderId],
		            trTestContext->mRecorderFileCount);
		...
		case T_RECORD_ONE_MP3_FILE_COMPLETE:
		{
			count++;
			printf(" file count %d\n", count);
			...
			snprintf(trTestContext->mRecorderPath, sizeof(trTestContext->mRecorderPath),
					"/mnt/UDISK/AW_%s_audio%d.mp3", num[trTestContext->mRecorderId],
					trTestContext->mRecorderFileCount);
...
static int AudioTest(RecoderTestContext *recorderTestContext, RecorderStatusContext *RecorderStatus, int number)
{
	/* Create TRecorder handle */
	memset(&recorderTestContext[number], 0, sizeof(RecoderTestContext));
	recorderTestContext[number].mTrecorder = CreateTRecorder();
	if (recorderTestContext[number].mTrecorder == NULL) {
		printf("CreateTRecorder[%d] err\n", number);
		return -1;
	}

	/* Reset Trecorder*/
	recorderTestContext[number].mRecorderId = number;
	TRreset(recorderTestContext[number].mTrecorder);

	TRsetAudioSrc(recorderTestContext[number].mTrecorder, T_AUDIO_MIC0);
    RecorderStatus[number].AudioMuteEnable = 0;

    /* only AAC format output is supported */
	TRsetOutput(recorderTestContext[number].mTrecorder,"/mnt/UDISK/audio.aac");
    TRsetMaxRecordTimeMs(recorderTestContext[number].mTrecorder, 60*1000);
    TRsetRecorderCallback(recorderTestContext[number].mTrecorder,
                            CallbackFromTRecorder,(void*)&recorderTestContext[number]);

	TRprepare(recorderTestContext[number].mTrecorder);
	TRstart(recorderTestContext[number].mTrecorder, T_RECORD);
    RecorderStatus[number].RecordingEnable = 1;
...
static int oneChannelTest(RecoderTestContext *recorderTestContext, RecorderStatusContext *RecorderStatus, int number)
{
	...
	i = number;
	/* Create TRecorder handle */
	memset(&recorderTestContext[i], 0, sizeof(RecoderTestContext));
	...
	TRsetCamera(recorderTestContext[i].mTrecorder, (TcameraIndex)i);
    RecorderStatus[i].VideoMarkEnable = 0;
	TRsetAudioSrc(recorderTestContext[i].mTrecorder, (TmicIndex)i);
    RecorderStatus[i].AudioMuteEnable = 0;
	if (access("/dev/disp", F_OK) == 0)
		TRsetPreview(recorderTestContext[i].mTrecorder,T_DISP_LAYER0);

    memset(outputchar, 0, sizeof(outputchar));
    sprintf(outputchar, "%s%s%s%s", path, "/mnt/UDISK/AW_", num[i], "_video0.mp4");
    TRsetOutput(recorderTestContext[i].mTrecorder,outputchar);
    TRsetMaxRecordTimeMs(recorderTestContext[i].mTrecorder, 60*1000);
	...
	if (access("/dev/disp", F_OK) == 0) {
		...
		ret = TRstart(recorderTestContext[i].mTrecorder,T_RECORD);
		if(ret < 0){
			printf("trecorder %d start error\n", i);
			return -1;
		}
	...
	if (access("/dev/disp", F_OK) == 0)
		...
		RecorderStatus[i].PreviewEnable= 0;
    RecorderStatus[i].RecordingEnable = 0;

    return 0;
}

static int twoChannelTest(RecoderTestContext *recorderTestContext, RecorderStatusContext *RecorderStatus)
{
	int i;
	TdispRect rect;
    char num[][6] = {"front", "rear"};
	...
	for (i = 0; i < 2; i++) {
        ...
        if (access("/dev/disp", F_OK) == 0) {
			if(i == 0) {
				TRsetPreview(recorderTestContext[i].mTrecorder,T_DISP_LAYER0);
				TRsetPreviewZorder(recorderTestContext[i].mTrecorder, VIDEO_ZORDER_BOTTOM);
			} else {
				TRsetPreview(recorderTestContext[i].mTrecorder,T_DISP_LAYER0);
				TRsetPreviewZorder(recorderTestContext[i].mTrecorder, VIDEO_ZORDER_TOP);
			}
		}
        memset(outputchar, 0, sizeof(outputchar));
        sprintf(outputchar, "%s%s%s", "/mnt/UDISK/AW_", num[i], "_video0.mp4");
        TRsetOutput(recorderTestContext[i].mTrecorder,outputchar);
        TRsetMaxRecordTimeMs(recorderTestContext[i].mTrecorder, 60*1000);
        TRsetRecorderCallback(recorderTestContext[i].mTrecorder,CallbackFromTRecorder,(void*)&recorderTestContext[i]);
	}
	...
	for (i = 0; i < 2; i++) {
		TRprepare(recorderTestContext[i].mTrecorder);
	}

	if (access("/dev/disp", F_OK) == 0) {
		for (i = 0; i < 2; i++) {
			TRstart(recorderTestContext[i].mTrecorder,T_PREVIEW);
			RecorderStatus[i].PreviewEnable= 1;
			RecorderStatus[i].RecordingEnable = 0;
		}
	} else {
...
int main(int argc, char** argv)
{
    ...
    if((argc == 3)&&(strcmp(argv[1],"audio")==0)){
        number = atoi(argv[2]);
        AudioTest(recorderTestContext, RecorderStatus, number);
	...
	if (number == 0 || number == 1)
		...
		twoChannelTest(recorderTestContext, RecorderStatus);
	else if (number == 3) {
		oneChannelTest(recorderTestContext, RecorderStatus, 0);
		number = 0;
		notCommand = 1;
	}
	else if (number == 4) {
		oneChannelTest(recorderTestContext, RecorderStatus, 1);
		number = 1;
		notCommand = 1;
	}
	else if (number == 5) {
		...
		if (readCommand(strCommandLine, sizeof(strCommandLine)) == 0) {
			...
			switch (nCommandId) {
				...
				case COMMAND_CAPTURE_PICTURE:
					...
					if (nCommandParam == 0) {
                        sprintf(captureConfig.capturePath, "/mnt/UDISK/jpeg_front%d.jpg", frontjpgNumber);
						captureConfig.captureWidth = 1920;
						captureConfig.captureHeight = 1080;

                        frontjpgNumber++;
					} else {
					    sprintf(captureConfig.capturePath, "/mnt/UDISK/jpeg_rear%d.jpg", rearjpgNumber);
