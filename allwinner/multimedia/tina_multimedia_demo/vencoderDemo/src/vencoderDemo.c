...
static const argument_t ArgumentMapping[] =
{
    ...
    { "-pformat",  "--piexl format",  FIXEL_FORMAT,
        "piexl format, 0:NV12, 1:NV21, 2:YU12, 3:YV12, 8: YUYV" },
...
static void PrintDemoUsage(void)
{
    ...
    printf("for h264 example: vencoderDemo -i /mnt/UDISK/decodeNV21_1920_1088_1.yuv -o /mnt/UDISK/h264.dat -s 1920x1088 -d 1920x1088 -n 1200 -f 0 -pformat 1\n");
    printf("for jpeg example: vencoderDemo -i /mnt/UDISK/decodeNV21_1920_1088_1.yuv -o /mnt/UDISK/jpeg.dat -s 1920x1088 -d 1920x1088 -n 1200 -f 1 -pformat 1\n");
}
...
void init_alter_frame_rate_info(VencAlterFrameRateInfo *pAlterFrameRateInfo)
{
    ...
    pAlterFrameRateInfo->sRoiBgFrameRate.nSrcFrameRate = 60;//25
    printf("[%s:%d]wht>>>>>>>>>>>>>>>>>warning, pAlterFrameRateInfo->sRoiBgFrameRate.nSrcFrameRate = %d\n", __func__, __LINE__,pAlterFrameRateInfo->sRoiBgFrameRate.nSrcFrameRate);
...
int setEncParam(VideoEncoder *pVideoEnc ,encode_param_t *encode_param)
{
    ...
    VideoEncSetParameter(pVideoEnc, VENC_IndexParamRotation, &encode_param->rotate);
...
void* ChannelThread(void* param)
{
    ...
    for(m=0; m<cycle_num; m++)
    {
        ...
        baseConfig.nStride = (encode_param.src_width + 15)& ~15;
        ...
        {
            unsigned int extSize = 1024;
            bufferParam.nSizeY = nAlignW*nAlignH*4 + extSize;
            bufferParam.nSizeC = 0;
            logd("input buffer: sizeY = %d, extSize = %d", bufferParam.nSizeY, extSize);
        }
        else
        {
        ...
        while(testNumber < encode_param.encode_frame_num)
        {
                       ...
                       || baseConfig.eInputFormat == VENC_PIXEL_UYVY422)
                    {
                        ...
                        if(size1!= bufferParam.nSizeY)
                        {
                            ...
                            size1 = fread(inputBuffer.pAddrVirY, 1, bufferParam.nSizeY, in_file);
                             ...
                             || baseConfig.eInputFormat == VENC_PIXEL_BGRA)
                    {
                        unsigned int dataSize = baseConfig.nInputWidth*baseConfig.nInputHeight*4;
                        ...
                        unsigned int size1;
                        size1 = fread(tmpBuf, 1,dataSize, in_file);
                        logv("**sizeY = %d, size1 = %d, w = %d, %d", dataSize, size1,
                             nAlignW, baseConfig.nInputWidth);
                        if(size1!= dataSize)
                        {
                            fseek(in_file, 0L, SEEK_SET);
                            size1 = fread(tmpBuf, 1, dataSize, in_file);
                        ...
                        unsigned int totalHeightNum = baseConfig.nInputHeight;
                        unsigned char* src = tmpBuf;
                        unsigned char* dst = inputBuffer.pAddrVirY;
                        for(i = 0; i< totalHeightNum;i++)
                        {
                            memcpy(dst, src, baseConfig.nInputWidth*4);
                            dst += nAlignW*4;
                            //dst += baseConfig.nInputWidth;
                            src += baseConfig.nInputWidth*4;
                        }
                    }
                    ...
                    else
                    {
                        ...
                        if(yu12_nv12_flag)
                        {
                            yu12_nv12(baseConfig.nInputWidth, baseConfig.nInputHeight,
                                 inputBuffer.pAddrVirC, uv_tmp_buffer, encode_param.encode_format);
                        }
            ...
            {
                inputBuffer.bEnableCorp = 1;
                inputBuffer.sCropInfo.nLeft =  encode_param.crop_l;
                inputBuffer.sCropInfo.nTop  =  encode_param.crop_t;
                inputBuffer.sCropInfo.nWidth  =  encode_param.crop_w;
                inputBuffer.sCropInfo.nHeight =  encode_param.crop_h;
            }
...
//#ifdef VBR
           ...
           if(pRoiConfig.bEnable)
            {
                   ...
                   for(idx=0; idx<pRoiConfig.num; idx++)
                   {
                        FILE *Roi_file;
                        char name[128];
                        sprintf(name, "/mnt/test/Roi_%u_%d.yuv", testNumber, idx);
        ...
        printf("encode costTime:avr=%0.2f ms,fps=%0.2f,min=%0.2f ms,max=%0.2f ms,cout=%d...\n",
             (float)avr,
             (float)1000/avr,
             (float)timeMin/1000,
