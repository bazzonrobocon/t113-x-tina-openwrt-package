...
get_network_stat()
{
    ...
    while [ $count -lt 6 ]; do
        if [ x"$( ./etc/wifi/gatewayip_check.sh | grep "Unreachable gatewayip")" != x"" ]; then
            echo "Unreachable gatewayip" > /dev/console
            count=$((count+1))
            sleep 5
        else
            return
    ...
    if [ $count -eq 6 ]; then
        echo "retry 6 times but failed, swupdate will exit!" > /dev/console
        exit
    fi
}
...
swupdate_cmd()
{
    if [ x"$(fw_printenv -n swu_param | grep "\-d")" != x"" ]; then
        get_network_stat
    fi
