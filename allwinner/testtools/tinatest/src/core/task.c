...
// *返回: 秒
// *========================================
int task_convert_time(char **str_array, int cnt) {
    int sec = 0;
    int index = 0;
    for (; index < cnt; index++) {
        switch (index) {
        ...
        enum mjson_type type)
{
    ...
    else if (!strcmp(key, SYSKEY_LOCAL_LIMIT_EXIT_TINATEST_WHEN_FAIL) && type == mjson_type_boolean) {
	DEBUG(DATA, "\n\n\nlimit: exit_tinatest_when_fail: %d => %d\n\n\n", lenv->limit.exit_tinatest_when_fail, val.m_boolean);
	lenv->limit.exit_tinatest_when_fail = val.m_boolean;
	/* need run alone now. */
        lenv->limit.run_alone = val.m_boolean;
    }
...
int task_create_reboot_status(struct task *task)
{
    FILE *fp = NULL;
    const char *fpath = REBOOT_STATUS;
    bool need_rename = false;

    if (!access(fpath, F_OK)) {
        fpath = REBOOT_STATUS "_tmp";
        need_rename = true;
    }
...
int task_create_fboot(void)
{
    ...
    {
        const char *autoboot = REBOOT_INIT;
        bool need_rename = false;

        if (!access(REBOOT_INIT, F_OK) || !access(REBOOT_INIT_LINK, F_OK)) {
            autoboot = REBOOT_INIT "_tmp";
            need_rename = true;
        }
        fp = fopen(autoboot, "w");
        if (fp == NULL) {
        ...
        if (need_rename == true) {
            if (rename(autoboot, REBOOT_INIT)) {
                ERROR("rename %s to %s failed\n", autoboot, REBOOT_INIT);
                goto out;
            }
        }

        if (access(REBOOT_INIT_LINK, F_OK)) {
...
{
    int ret = -1;
    char *buf = NULL, *new_buf = NULL, *p = NULL, *pp[ARRAY_MAX];
...
int task_init_reboot_mode(struct task *task)
{
    ...
    if (task_create_reboot_status(task) != 0)
        return -1;
...
void *task_recycle(void *arg)
{
    ...
    int status;
    pid_t pid;
    struct task *pre = NULL, *next = NULL, *task = NULL;
...
again:
        ...
        list_for_each_entry_safe(pre, next, &TASK_LIST, lnode) {
		    ...
		    if (pre->result[num] != 0) {
			    DEBUG(BASE, "env.limit.exit_tinatest_when_fail: %d...\n\n", pre->env.limit.exit_tinatest_when_fail);
			    if (pre->env.limit.exit_tinatest_when_fail == false)
				    continue;

			    struct task *tt = NULL;
			    list_for_each_entry(tt, &TASK_LIST, lnode) {
				    if (!strcmp(pre->keypath, tt->keypath))
					    continue;
				    task_del(tt->keypath);
			    }
			    goto out;
    ...
    if (is_existed(REBOOT_STATUS) == true) { //existed
        ...
        if ((bytes = getline(&fstdout, &len, fp)) == -1) {
            ...
            if (is_existed(fstdout) == false) {
                ...
                fstdout = (char *)realloc(fstdout, strlen(SERIAL + 4));
        ...
        if ((bytes = getline(&fstderr, &len, fp)) == -1) {
            ...
            if (is_existed(fstderr) == false) {
                ERROR("Recover fstdout after reboot: Not Found %s, replace as %s\n", fstderr, SERIAL);
                fstderr = (char *)realloc(fstderr, strlen(SERIAL + 4));
