...
static int request_rv_yuv_num = 0;
...
#ifdef __SAVE_RV_YUV__
static int save_rv_yuv(camera_handle *camera, int request_rv_yuv_num)
{
	struct rv_yuv_mmap_cfg rv_yuv_mmap;
	struct get_rv_yuv_cfg get_rv_yuv;
	char source_data_path[128];
	int get_frame_num;

	memset(&rv_yuv_mmap, 0, sizeof(struct rv_yuv_mmap_cfg));

	rv_yuv_mmap.en = 1;
	if (ioctl(camera->videofd, VIDIOC_SET_RV_YUV_MMAP_CFG, &rv_yuv_mmap) != 0) {
		camera_err("cannot set mmap en 1 cfg\n");
		return -1;
	}

	get_frame_num = 0;
	while(get_frame_num < request_rv_yuv_num) {
		memset(&get_rv_yuv, 0, sizeof(struct get_rv_yuv_cfg));
		get_rv_yuv.request_frame_num = 1;
		if (ioctl(camera->videofd, VIDIOC_GET_RV_YUV_CFG, &get_rv_yuv) != 0) {
			camera_err("get rv yuv fail!\n");
			goto SAVE_RV_YUV_EXIT;
		} else {
			if(get_rv_yuv.is_frame_available) {
				printf("save rv frame %d\n", get_frame_num+1);
				sprintf(source_data_path, "%s/rv_source_%d.yuv", camera->save_path, get_frame_num);
				save_frame_to_file(source_data_path, get_rv_yuv.vir_addr, get_rv_yuv.size);

			} else {
				if(get_rv_yuv.is_scan_stop) {
					camera_print("scan stop, no more frame, exit!\n");
					break;
				} else {
					camera_dbg("wait rv frame");
				}
			}
		}
		get_frame_num++;
...
SAVE_RV_YUV_EXIT:
	...
	rv_yuv_mmap.en = 0;
	if (ioctl(camera->videofd, VIDIOC_SET_RV_YUV_MMAP_CFG, &rv_yuv_mmap) != 0) {
		camera_err("cannot set mmap en 0 cfg\n");
		return -1;
	}
	close(camera->videofd);
	request_rv_yuv_num = 0;
...
int main(int argc, char *argv[])
{
	...
	if ((argc == 2) && (strcmp(argv[1], "setting") == 0)) {
		...
		camera.photo_num = 20;
	} else if ((argc == 5) && (strcmp(argv[1], "rv") == 0)) {
		camera.camera_index = atoi(argv[2]);
		request_rv_yuv_num = atoi(argv[3]);
		memset(camera.save_path, 0, sizeof(camera.save_path));
		memcpy(camera.save_path, argv[4], sizeof(camera.save_path));
...
#ifdef __SAVE_RV_YUV__
	if(request_rv_yuv_num != 0) {
		return save_rv_yuv(&camera, request_rv_yuv_num);
	}
#endif
