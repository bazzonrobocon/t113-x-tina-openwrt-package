...
enum wmgState wifi_sta_status = STATE_UNKNOWN;
...
void va_wifi_msg_cb(wifi_msg_data_t *msg)
{
	com_info("mod:%d event:%d\n", msg->id, msg->data.event);

	if(WIFI_MSG_ID_STA_CN_EVENT == msg->id)
	{
		switch(msg->data.event) {
		case WIFI_CONNECTED:
		case WIFI_DHCP_SUCCESS:
			wifi_sta_status = NETWORK_CONNECTED;
			break;
		default :
			wifi_sta_status = DISCONNECTED;
			break;
		}
	}
}
