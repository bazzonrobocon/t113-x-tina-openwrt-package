...
static int diskmanager_compare_hotplug_message(void *data, void *compare_param)
{
	hotplug_disk_message_t *hotplug_message = (hotplug_disk_message_t *)data;
	hotplug_disk_message_t *hotplug_message_compare_info = (hotplug_disk_message_t *)compare_param;

//	com_err("DeviceName:%s\n", disk_tmp->DeviceName);
//	com_err("MountPoint:%s\n", disk_tmp->MountPoint);
	if (strlen(hotplug_message->diskinfo->DeviceName) == strlen(hotplug_message_compare_info->diskinfo->DeviceName) &&
		0 == strncmp(hotplug_message->diskinfo->DeviceName, hotplug_message_compare_info->diskinfo->DeviceName, strlen(hotplug_message_compare_info->diskinfo->DeviceName)) &&
		strlen(hotplug_message->diskinfo->MountPoint) == strlen(hotplug_message_compare_info->diskinfo->MountPoint) &&
		0 == strncmp(hotplug_message->diskinfo->MountPoint, hotplug_message_compare_info->diskinfo->MountPoint, strlen(hotplug_message_compare_info->diskinfo->MountPoint))) {
		return 0;
	}
	return 1;
}
...
static void DiskManager_Filter_HotPlug_Message_Info(hotplug_disk_message_t *hotplug_message) {
	hotplug_disk_message_t *hotplug_message_tmp = NULL;
	hotplug_message_tmp = __db_list_search_and_pop(hotplug_message_list, hotplug_message, diskmanager_compare_diskinfo);
	if (hotplug_message_tmp != NULL) {
		free(hotplug_message_tmp);
		hotplug_message_tmp = NULL;
	}
}
static void DiskManager_HotPlug_Message_Info(hotplug_disk_message_t *hotplug_message) {
	__db_list_put_tail(hotplug_message_list, hotplug_message);
}
...
static int DiskManager_Clean_MountPoint(void) {
	...
	for (;Index < sizeof(UsbMountPoint)/sizeof(UsbMountPoint[0]); Index++) {
		...
		if (UsbMountPoint[Index].NeedCreate_Clean) {
			char Cmd[64] = {0};
			snprintf(Cmd, sizeof(Cmd), "rm -rf %s", UsbMountPoint[Index].MountPoint);
			system(Cmd);
		}
	}
	...
	for (;Index < sizeof(SdcardMountPoint)/sizeof(SdcardMountPoint[0]); Index++) {
		...
		if (SdcardMountPoint[Index].NeedCreate_Clean) {
			char Cmd[64] = {0};
			snprintf(Cmd, sizeof(Cmd), "rm -rf %s", SdcardMountPoint[Index].MountPoint);
			system(Cmd);
		}
	}
	return 0;
	...
	if (DeviceInfo->MediaType == MEDIUM_USB_MASSSTORAGE) {
		for (;Index < sizeof(UsbMountPoint)/sizeof(UsbMountPoint[0]); Index++) {
			if (DiskManager_MountPoint_Is_Used(UsbMountPoint[Index].MountPoint) == 0) {
				if (UsbMountPoint[Index].NeedCreate_Clean) {
					char Cmd[64] = {0};
					snprintf(Cmd, sizeof(Cmd), "mkdir -p %s", UsbMountPoint[Index].MountPoint);
					system(Cmd);
				}
		...
		for (;Index < sizeof(SdcardMountPoint)/sizeof(SdcardMountPoint[0]); Index++) {
			if (DiskManager_MountPoint_Is_Used(SdcardMountPoint[Index].MountPoint) == 0) {
				if (SdcardMountPoint[Index].NeedCreate_Clean) {
					char Cmd[64] = {0};
					snprintf(Cmd, sizeof(Cmd), "mkdir -p %s", SdcardMountPoint[Index].MountPoint);
					system(Cmd);
				}
...
static int DiskManager_MountDevice(DiskInfo_t *DeviceInfo, int bootcheck) {
	...
	hotplug_disk_message_t *hotplug_message = NULL;
	if (DiskManager_Check_DiskMountStatus(DeviceInfo) == 0)
	{
		...
		for (Index = 0; Index < sizeof(FileSystemType)/sizeof(FileSystemType[0]); Index++) {
			if (0) {
				...
				char command[128] = {0};
				if (strcmp(FileSystemType[Index], "vfat") == 0 || strcmp(FileSystemType[Index], "ntfs-3g") == 0) {
					snprintf(command, 128, "mount -t %s -o time_offset=480 %s %s", FileSystemType[Index], DeviceInfo->DeviceName, DeviceInfo->MountPoint);
				} else {
...
//				com_err("mount fail:%s\n", strerror(errno));
		...
		hotplug_message = (hotplug_disk_message_t *)malloc(sizeof(hotplug_disk_message_t));
		if (DiskManager_Item == NULL || NULL == hotplug_message) {
			if (DiskManager_Item) {
				free(DiskManager_Item);
				DiskManager_Item = NULL;
			}
			if (hotplug_message) {
				free(hotplug_message);
				hotplug_message = NULL;
			}
		...
		if (bootcheck == 0) {
			hotplug_message->operate = MEDIUM_PLUGIN;
		} else {
			hotplug_message->operate = MEDIUM_STATIC_PLUGIN;
		}
		...
		hotplug_message->diskinfo = DiskManager_Item;
		DiskManager_HotPlug_Message_Info(hotplug_message);
	} else {
		com_err("%s already mount", DeviceInfo->DeviceName);
		com_err("mountinfo:");
		system("mount");
	}
	return 0;
...
static int DiskManager_UnMountDevice(DiskInfo_t *DeviceInfo) {
	...
	hotplug_message = (hotplug_disk_message_t *)malloc(sizeof(hotplug_disk_message_t));
	if (NULL == hotplug_message) {
		com_err("malloc fail\n");
		return -1;
	}
	hotplug_message->operate = MEDIUM_PLUGOUT;
	hotplug_message->diskinfo = DiskManager_Item;
//	DiskManager_Filter_HotPlug_Message_Info(hotplug_message);
...
{
	hotplug_message_focus_win_t *RegisterInfo = (hotplug_message_focus_win_t *)data;
	hotplug_disk_message_t *hotplug_message = (hotplug_disk_message_t *)param;
	...
	if (RegisterInfo->status) {
		...
		LOG_INFO("Cur_Win:%s callback end\n", RegisterInfo->Cur_Win);
	}
