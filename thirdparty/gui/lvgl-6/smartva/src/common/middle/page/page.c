...
static inline int compare_by_page(void *dst, void *src)
{
	if(dst == NULL || src == NULL){
		printf("[%s: %d] paragarms error\n", __FILE__, __LINE__);
		return -1;
	...
	page_interface_t *dst_page = (page_interface_t *)dst;
	page_interface_t *src_page = (page_interface_t *)src;
...
void page_init()
{
	...
	page_msg_head = db_list_create("page_msg_head", 1);
...
{
	assert(page_head != NULL);
	__db_list_search_and_pop(page_head, page, compare_by_page);
}
...
{
	if (is_list_empty(page_msg_head)) {
		return current_id;
	} else {
		page_param *param = __db_list_search_node_byindex(page_msg_head, __db_list_get_num(page_msg_head) - 1);
