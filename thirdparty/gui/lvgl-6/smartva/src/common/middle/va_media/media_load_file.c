...
void media_set_play_file_index(media_file_list_t *media_file_list, int index)
{
	...
	rat_npl_set_cur(media_file_list->media_hrat, media_file_list->play_index);
}
...
int media_get_next_play_index(media_file_list_t *media_file_list)
{
    int index = -1;
    HRAT media_hrat = media_file_list->media_hrat;
    int play_mod = rat_npl_get_play_mode(media_hrat);
    int media_total_num = media_file_list->total_num;

    index = rat_npl_get_next(media_hrat);
    if (play_mod == RAT_PLAY_MODE_ONLY_ONCE && index == -1)
    {
        index = media_get_play_file_index(media_file_list) + 1;
    }
    else if (play_mod == RAT_PLAY_MODE_SEQUENCE && index == -1)
    {
        index = 0;
    }
    else if (play_mod == RAT_PLAY_MODE_ROTATE_ONE)
    {
        index++;
    }

    index = (index >= media_total_num) ? 0 : index;
    com_info ("play_mod = %d media_total_num = %d index = %d", play_mod, media_total_num, index);
    return index;
}

int media_get_last_play_index(media_file_list_t *media_file_list)
{
    int index = -1;
    HRAT media_hrat = media_file_list->media_hrat;
    int play_mod = rat_npl_get_play_mode(media_hrat);
    int media_total_num = rat_npl_get_total_count(media_hrat);

    index = rat_npl_get_prev(media_hrat);
    if ((play_mod == RAT_PLAY_MODE_ONLY_ONCE ||
         play_mod == RAT_PLAY_MODE_SEQUENCE) && index == -1)
    {
        index = media_get_play_file_index(media_file_list) - 1;
    }
    else if (play_mod == RAT_PLAY_MODE_ROTATE_ONE)
    ...
    index = (index < 0) ? media_total_num - 1 : index;
    com_info ("play_mod = %d media_total_num = %d index = %d", play_mod, media_total_num, index);
    return index;
}
