...
static int image_save_file(image_buffer_t *src, const char *file_name)
{
	...
	ret = fwrite(src->vir_addr, src->length, 1, fp);
...
int image_rotate(image_rotate_angle_t rotate_angle)
{
	...
	if(image.src.frame_cnt > 1)//gif不支持动态旋转
	{
		return 0;
	}
...
int image_scaler(int *ratio_percent)
{
	...
	int ret;
	if(image.src.frame_cnt > 1)//gif不支持动态缩放
	{
		return 0;
	}
...
int image_decode(char *filename, image_buffer_t *buf)
{
	...
	width = abs(width);
	height = abs(height);
	if(width*height*comp > DECODE_LIMIT_SIZE)//限制一下解码大小
	{
	...
	else
	{
		...
		if(is_gif(filename))
		{
			...
			image.src.vir_addr = ionHdle->palloc(width * height * comp);
			image.src.phy_addr = ionHdle->cpu_get_phyaddr(image.src.vir_addr);
			ret = gif_decode_sw(filename,image.src.vir_addr,&buf->width,&buf->height,&comp);
			printf("buf1 w = %d h = %d ret = %d\n",buf->width,buf->height,ret);
			if(ret == 0 && buf->width == width && buf->height == height)
			{
				buf->comp = 4;
...
int image_show(char *filename, image_show_mode_t mode)
{
	...
	{
		if(image.src.frame_cnt == 1 || retval == -1)
		{
			image_show_before();
			crop_rect.x = 0;
			crop_rect.y = 0;
		...
		{
			gif_play_start(filename,GIF_MODE_CIRCLE);
			gif_wait_first_frame();
		}
	}
...
int image_init(void)
{
	...
	image.dst_previous.width  = disp_info.screen_width;
	image.dst_previous.height = disp_info.screen_height;
	...
	image.dst_next.width  = disp_info.screen_width;
	image.dst_next.height = disp_info.screen_height;
