...
int va_save_param_init(void)
{
	memset(&va_save_param, 0x00, sizeof(va_save_param));
	va_save_param.config_path = VA_SAVE_PATH;
	va_save_param.backup_config_path = VA_BACKUP_SAVE_PATH;
	return init_cjson_config(&va_save_param);
}
...
{
	int index = 0, need_reset = 0;

	if (!reset) {
		need_reset = va_save_param_init();
	}
