...
void power_off_process(void) {
	com_info("slepp 3s to power off");
	sleep(3);
	system("sync");
...
void get_file_time_info(const char *FilePath, char *string, int len) {
	...
	if (buf.f_type == EXFAT_SUPER_MAGIC) {
		strftime(string, len, "%Y-%m-%d %H:%M:%S", gmtime(&file_stat.st_mtime));
	} else {
		strftime(string, len, "%Y-%m-%d %H:%M:%S", localtime(&file_stat.st_mtime));
	}
}
static void Filesize2str(__u32 size, char *str)
{
	...
	// С�� 1 G
		...
		sprintf(str, "%d.%d%d M", size / (1024 * 1024),
			((size % (1024 * 1024)) / 1024) * 1000 / 1024 / 100,
			((size % (1024 * 1024)) / 1024) * 1000 / 1024 % 100 /
			    10); //������λС��
	} else {
		sprintf(str, "%d.%d%d G", size / (1024 * 1024 * 1024),
			(size % (1024 * 1024 * 1024)) / (1024 * 1024) * 1000 /
			    1024 / 100,
			(size % (1024 * 1024 * 1024)) / (1024 * 1024) * 1000 /
			    1024 % 100 / 10);
	}
}
void get_file_size_info(const char *FilePath, char *string, int len) {
	struct stat file_stat;

	stat(FilePath, &file_stat);
	Filesize2str(file_stat.st_size, string);
}
