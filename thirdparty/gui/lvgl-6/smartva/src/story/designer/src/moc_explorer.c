...
static ui_image_t explorer_image_list[EXPLORER_IMAGE_NUM_MAX] = {
    {NULL, EXPLORER_IMAGE_AUDIO_PATH},   {NULL, EXPLORER_IMAGE_VIDEO_PATH},
    {NULL, EXPLORER_IMAGE_PICTURE_PATH}, {NULL, EXPLORER_IMAGE_FIRMWARE_PATH},
    {NULL, EXPLORER_IMAGE_FOLDER_PATH},  {NULL, EXPLORER_IMAGE_TEXT_PATH},
    {NULL, EXPLORER_IMAGE_USB_PATH},     {NULL, EXPLORER_IMAGE_SD_PATH},
    {NULL, EXPLORER_IMAGE_MORE_PATH},    {NULL, EXPLORER_IMAGE_UNKOWN_PATH},
...
static int Explorer_TypeAll_Update_Filelist(file_list_t *cur_list, int start,
					    int end);
...
static bool __is_in_obj(lv_obj_t *obj, __u32 x, __u32 y)
{
	...
	if (x > obj_area.x2 || x < obj_area.x1 || y > obj_area.y2 ||
	    y < obj_area.y1) {
		return false;
	} else {
		return true;
...
static void Explorer_Release_CurFileList(void)
{
	...
	while (Child != NULL &&
	       Child != &Explorer_Para.explorer_list->empty_file_list) {
		Temp = Child;
		Child = Child->child;
...
static void Explorer_Update_FileInfo(const char *FilePath)
{
	...
	get_file_size_info(FilePath, str, sizeof(str));
#ifdef LV_USE_LABEL
	lv_obj_set_hidden(Explorer_Para.ui.File_Size, 0);
	//	printf("%s %d %s str:%s\n", __FILE__, __LINE__, __func__, str);
	lv_label_set_text(Explorer_Para.ui.File_Size, str);
#endif // LV_USE_LABEL

	memset(str, 0x00, sizeof(str));
	get_file_time_info(FilePath, str, sizeof(str));
#ifdef LV_USE_LABEL
	lv_obj_set_hidden(Explorer_Para.ui.file_time, 0);
	lv_label_set_text(Explorer_Para.ui.file_time, str);
#endif // LV_USE_LABEL
	return;
}
static void Explorer_Back_Btn_Event(lv_obj_t *btn, lv_event_t event)
{
	if (event == LV_EVENT_RELEASED) {
		...
		if (Explorer_Para.explorer_list->cur_file_list->parent !=
			Explorer_Para.explorer_list->top_file_list &&
		    Explorer_Para.explorer_list->cur_file_list !=
			Explorer_Para.explorer_list->top_file_list) {
			...
			Explorer_Para.explorer_list->cur_file_list = parent;
		} else if (Explorer_Para.explorer_list->cur_file_list ==
			   &Explorer_Para.explorer_list->empty_file_list) {
			printf("do nothing\n");
			return;
		} else {
			printf("this top dir\n");
		...
		end = ((start + EXPLORER_LIST_TASKLOAD_ITEM_NUM) >
		       Explorer_Para.explorer_list->cur_file_list->total)
			  ? Explorer_Para.explorer_list->cur_file_list->total
			  : (start + EXPLORER_LIST_TASKLOAD_ITEM_NUM);
		Explorer_TypeAll_Update_Filelist(
		    Explorer_Para.explorer_list->cur_file_list, start, end);
		//		printf("%s %d %s file_path:%s\n", __FILE__,
		//__LINE__, __func__,
		//Explorer_Para.explorer_list->cur_file_list->file_path);
		Explorer_Update_Directory(
		    Explorer_Para.explorer_list->cur_file_list->file_path);
		if (end < Explorer_Para.explorer_list->cur_file_list->total) {
			auto_load_task =
			    lv_task_create((void *)Explorer_AutoLoad_Task, 100,
					   LV_TASK_PRIO_MID, NULL);
		}
	}
}
static __s32 Explorer_Add_FileListItem(rat_media_type_t type,
				       const char *FileName)
{
...
//	printf("%s %d %s type:%d FileName:%s\n", __FILE__, __LINE__, __func__, type, FileName);
	switch (type) {
	case RAT_MEDIA_TYPE_PIC:
		img_src = get_image_buff_form_list(explorer_image_list,
						   EXPLORER_IMAGE_NUM_MAX,
						   EXPLORER_IMAGE_PICTURE);
		btn_temp =
		    lv_list_add_btn(Explorer_Para.ui.list_2, img_src, FileName);
		break;
	case RAT_MEDIA_TYPE_AUDIO:
		img_src = get_image_buff_form_list(explorer_image_list,
						   EXPLORER_IMAGE_NUM_MAX,
						   EXPLORER_IMAGE_AUDIO);
		btn_temp =
		    lv_list_add_btn(Explorer_Para.ui.list_2, img_src, FileName);
		break;
					    ...
					    int end)
{
	...
	lv_obj_set_hidden(Explorer_Para.ui.File_Size, 1);
	lv_obj_set_hidden(Explorer_Para.ui.file_time, 1);
	return 0;
}
static void Explorer_FileList_SwitchToApp(int Index)
{
	...
	if (Explorer_Para.rat_ctrl.media_type == RAT_MEDIA_TYPE_ALL) {
		...
		file_item = get_file_list_item(
		    Explorer_Para.explorer_list->cur_file_list, Index);
		...
		ret = rat_get_item_info_by_index(Explorer_Para.rat_ctrl.handle,
						 Index, &ItemInfo);
		if (ret == EPDK_FAIL) {
	...
	for (i = 0; i < count; i++) {
		...
		if (strncmp(disk_tmp->MountPoint, FilePath,
			    strlen(disk_tmp->MountPoint)) == 0) {
...
static void Explorer_FileList_event(lv_obj_t *btn, lv_event_t event)
{
	if (event == LV_EVENT_SHORT_CLICKED) {
		...
		/*����list�ڶ��˲��ܴ��������¼�������������жϵ��λ���Ƿ���Ԫ����*/
		if (__is_in_obj(btn, indev->proc.types.pointer.act_point.x,
				indev->proc.types.pointer.act_point.y)) {
			unsigned int Index =
			    lv_list_get_btn_index(Explorer_Para.ui.list_2, btn);

			if (Explorer_Para.rat_ctrl.media_type ==
			    RAT_MEDIA_TYPE_ALL) {
				file_item_t *sel_item = get_file_list_item(
				    Explorer_Para.explorer_list->cur_file_list,
				    Index);
				if (sel_item->fatdirattr ==
				    FSYS_ATTR_DIRECTORY) {
					Explorer_Para.explorer_list->file_item =
					    sel_item;
				...
				} else //��������ļ����͸����ļ���Ϣ
				{
					char FilePath[RAT_MAX_FULL_PATH_LEN +
						      16] = {0};
					sprintf(FilePath, "%s/%s",
						Explorer_Para.explorer_list
						    ->cur_file_list->file_path,
						sel_item->name);
				...
				memset(&ItemInfo, 0, sizeof(rat_entry_t));
				ret = rat_get_item_info_by_index(
				    Explorer_Para.rat_ctrl.handle, Index,
				    &ItemInfo);
				if (ret == EPDK_FAIL) {
					printf("get file information form rat "
					       "fail!!");
					return;
				}
...
static void Explorer_AutoLoad_Task(lv_task_cb_t *task)
{
	if (Explorer_Para.rat_ctrl.media_type == RAT_MEDIA_TYPE_ALL) {
		file_list_t *cur_list =
		    Explorer_Para.explorer_list->cur_file_list;
		unsigned int list_btn_num =
				 lv_list_get_size(Explorer_Para.ui.list_2),
			     start = 0, end = 0;

		start = list_btn_num;
		end = ((list_btn_num + EXPLORER_LIST_TASKLOAD_ITEM_NUM) >
		       cur_list->total)
			  ? cur_list->total
			  : (list_btn_num + EXPLORER_LIST_TASKLOAD_ITEM_NUM);
		...
		if (!rat_is_cursor_end(Explorer_Para.rat_ctrl.handle)) {
			...
			rat_move_cursor_forward(
			    Explorer_Para.rat_ctrl.handle,
			    EXPLORER_LIST_TASKLOAD_ITEM_NUM);
			Explorer_Para.rat_ctrl.total =
			    rat_get_cur_scan_cnt(Explorer_Para.rat_ctrl.handle);
			//			printf("%s %d %s
			//Explorer_Para.rat_ctrl.total:%d\n",__FILE__,__LINE__,__func__,
			//Explorer_Para.rat_ctrl.total);
			for (; Index < Explorer_Para.rat_ctrl.total; Index++) {
				int ret = 0;
				rat_entry_t ItemInfo;
				extern char *SLIB_strchrlast(char *pstr,
							     char srch_char);
				...
				//				printf("%s %d %s
				//SLIB_strchrlast(ItemInfo.Path, '/') +
				//1:%s\n",__FILE__,__LINE__,__func__,
				//SLIB_strchrlast(ItemInfo.Path, '/') + 1);
				Explorer_Add_FileListItem(
				    Explorer_Para.rat_ctrl.media_type,
				    SLIB_strchrlast(ItemInfo.Path, '/') + 1);
			}
		} else {
			lv_task_del(auto_load_task);
...
static void Explorer_FileList_Update(void)
{
	if (Explorer_Para.rat_ctrl.media_type == RAT_MEDIA_TYPE_ALL) {
		...
		/*打开第一个设备，进入第一级目录*/
		if (Explorer_Para.explorer_list &&
		    Explorer_Para.explorer_list->cur_file_list &&
		    Explorer_Para.explorer_list->cur_file_list->file_path &&
		    strlen(Explorer_Para.explorer_list->cur_file_list
			       ->file_path) != 0) {
			sprintf(file_path, "%s/",
				Explorer_Para.explorer_list->cur_file_list
				    ->file_path);
			//			printf("%s %d %s file_path:%s\n",
			//__FILE__, __LINE__, __func__, file_path);
		}
		...
		if (Explorer_Para.explorer_list &&
		    Explorer_Para.explorer_list->file_item &&
		    Explorer_Para.explorer_list->file_item->name) {
			//			printf("%s %d %s name:%s\n",
			//__FILE__, __LINE__, __func__,
			//Explorer_Para.explorer_list->file_item->name);
			strcat(file_path,
			...
			if (FileList != NULL) {
				...
				Explorer_TypeAll_Update_Filelist(
				    ...
				    start, end);
				//开始预加载任务
				if (end < Explorer_Para.explorer_list
					      ->cur_file_list->total) {
					auto_load_task = lv_task_create(
					    (void *)Explorer_AutoLoad_Task, 100,
					    LV_TASK_PRIO_MID, NULL);
				}
			} else {
				lv_list_clean(Explorer_Para.ui.list_2);
				memset(&Explorer_Para.explorer_list
					    ->empty_file_list,
				       0x00, sizeof(file_list_t));
				Explorer_Para.explorer_list->empty_file_list
				    .parent =
				    Explorer_Para.explorer_list->cur_file_list;
				Explorer_Para.explorer_list->cur_file_list
				    ->child = &Explorer_Para.explorer_list
						   ->empty_file_list;
				Explorer_Para.explorer_list->cur_file_list =
				    &Explorer_Para.explorer_list
					 ->empty_file_list;
			...
			lv_list_clean(Explorer_Para.ui.list_2);
			memset(&Explorer_Para.explorer_list->empty_file_list,
			       0x00, sizeof(file_list_t));
			Explorer_Para.explorer_list->empty_file_list.parent =
			    Explorer_Para.explorer_list->cur_file_list;
			Explorer_Para.explorer_list->cur_file_list->child =
			    &Explorer_Para.explorer_list->empty_file_list;
			Explorer_Para.explorer_list->cur_file_list =
			    &Explorer_Para.explorer_list->empty_file_list;
			//		printf("%s %d %s file_path:%s\n",
		...
		strcpy(Explorer_Para.rat_ctrl.SearchPath,
		       Explorer_Para.explorer_list->file_item->name);
		Explorer_Para.rat_ctrl.handle =
		    rat_open(Explorer_Para.rat_ctrl.SearchPath,
			     Explorer_Para.rat_ctrl.media_type,
			     EXPLORER_LIST_PERLOAD_ITEM_NUM); //全部搜索
		Explorer_Para.rat_ctrl.total =
		    rat_get_cur_scan_cnt(Explorer_Para.rat_ctrl.handle);
		lv_list_clean(Explorer_Para.ui.list_2);
		for (Index = 0; Index < Explorer_Para.rat_ctrl.total; Index++) {
			...
			memset(&ItemInfo, 0, sizeof(rat_entry_t));
			ret = rat_get_item_info_by_index(
			    Explorer_Para.rat_ctrl.handle, Index, &ItemInfo);
			if (ret == EPDK_FAIL) {
				printf("get file information form rat fail!!");
				return;
			}
...
static void Explorer_MediumList_Event(lv_obj_t *btn, lv_event_t event)
{
	...
	if (event == LV_EVENT_SHORT_CLICKED) {
		...
		if (__is_in_obj(btn, indev->proc.types.pointer.act_point.x,
			...
			CurFocus_btn_obj = lv_list_get_btn_selected(
			    Explorer_Para.ui.medium_list_1);
			CurFocusItem = lv_list_get_btn_index(
			    Explorer_Para.ui.medium_list_1, CurFocus_btn_obj);
			CurItem = lv_list_get_btn_index(
			    Explorer_Para.ui.medium_list_1, btn);
			if (CurFocusItem == CurItem) {
				printf("not need to update\n");
				return;
			}
			lv_list_set_btn_selected(Explorer_Para.ui.medium_list_1,
						 btn);
			Explorer_Para.explorer_list->file_item =
			    get_file_list_item(
				Explorer_Para.explorer_list->top_file_list,
...
static void Explorer_MediumList_AddItem(unsigned int FocusItem,
					unsigned int Index)
{
	file_item_t *file_item = NULL;
	rat_media_type_t type;
	...
	file_item =
	    get_file_list_item(Explorer_Para.explorer_list->top_file_list,
			       Index); //��node��������Ϣ
	type = get_file_list_item_file_type(file_item);
	switch (type) {
	...
	btn_tmp = lv_list_add_btn(Explorer_Para.ui.medium_list_1, img_src,
				  get_text_by_id(string_id));
	...
	if (FocusItem == Index) {
		...
		btn_obj =
		    lv_list_get_btn_selected(Explorer_Para.ui.medium_list_1);
		if (btn_obj) {
			lv_btn_set_state(btn_obj, LV_BTN_STATE_REL);
		}
		Explorer_Para.explorer_list->file_item = get_file_list_item(
		    Explorer_Para.explorer_list->top_file_list, FocusItem);
		lv_list_set_btn_selected(Explorer_Para.ui.medium_list_1,
					 btn_tmp);
	}
...
static void Explorer_MediumList_DelItem(unsigned int Index)
{
	...
	focus_index =
	    lv_list_get_btn_index(Explorer_Para.ui.medium_list_1, current_obj);
	Explorer_Para.explorer_list->file_item = get_file_list_item(
	    Explorer_Para.explorer_list->top_file_list, focus_index);
}
...
static void Explorer_MediumList_Init(char *MountPoint)
{
	...
	Explorer_Para.explorer_list =
	    (explorer_list_t *)malloc(sizeof(explorer_list_t));
	memset(Explorer_Para.explorer_list, 0, sizeof(explorer_list_t));
	/*Left list*/
	Explorer_Para.explorer_list->top_file_list =
	    new_file_root_list_nod(NULL);
	if (MountPoint) {
		FocusItem = FileList_GetItemNum_ByMountPoint(
		    Explorer_Para.explorer_list->top_file_list, MountPoint);
	} else {
		FocusItem = 0;
	}
	if (Explorer_Para.explorer_list->top_file_list) {
		for (Index = 0;
		     Index < Explorer_Para.explorer_list->top_file_list->total;
		     Index++) {
...
static void Explorer_DiskHotPlugCallBack(hotplug_disk_message_t *hotplug_message)
{
	pthread_mutex_lock(&Explorer_HotPlug_Mutex);
	if (hotplug_message->operate != MEDIUM_PLUGOUT) {
		FileList_AddFileNod_To_RootList(
		    Explorer_Para.explorer_list->top_file_list, hotplug_message->diskinfo);
		Explorer_MediumList_AddItem(
		    Explorer_Para.explorer_list->top_file_list->total - 1,
		    Explorer_Para.explorer_list->top_file_list->total - 1);
		Explorer_MediumList_Update();
	} else {
		...
		ItemNum = FileList_GetItemNum_ByMountPoint(
		    Explorer_Para.explorer_list->top_file_list,
		...
		if (ItemNum == CurFocusItem) {
			if (CurFocusItem == 0) {
				NewFocus_btn_obj = lv_list_get_next_btn(
				    Explorer_Para.ui.medium_list_1,
				    CurFocus_btn_obj);
			} else {
				NewFocus_btn_obj = lv_list_get_prev_btn(
				    Explorer_Para.ui.medium_list_1,
				    CurFocus_btn_obj);
			}
			if (NewFocus_btn_obj) {
				...
				printf(
				    "not disk to explorer will return to home");
				pthread_mutex_unlock(&Explorer_HotPlug_Mutex);
				switch_page(PAGE_EXPLORER, PAGE_HOME);
				return;
			}
		}
		FileList_DelFileNod_To_RootList(
		    Explorer_Para.explorer_list->top_file_list, hotplug_message->diskinfo);
...
static void explorer_label_text_init(explorer_ui_t *ui)
{
	...
	lv_label_set_text(ui->label_1,
			  get_text_by_id(LANG_EXPLORER_INFOMATION));
	lv_label_set_text(ui->label_4, get_text_by_id(LANG_EXPLORER_FILE_SIZE));
	lv_label_set_text(ui->label_7,
			  get_text_by_id(LANG_EXPLORER_FILE_MODIFIED_TIME));
	...
	memcpy(&list_style_rel,lv_list_get_style(ui->list_2, LV_LIST_STYLE_BTN_REL),sizeof(lv_style_t));
	memcpy(&list_style_pr,lv_list_get_style(ui->list_2, LV_LIST_STYLE_BTN_PR),sizeof(lv_style_t));
	memcpy(&list_style_tgl_rel,lv_list_get_style(ui->list_2, LV_LIST_STYLE_BTN_TGL_REL),sizeof(lv_style_t));
	memcpy(&list_style_tgl_pr,lv_list_get_style(ui->list_2, LV_LIST_STYLE_BTN_TGL_PR),sizeof(lv_style_t));
	...
	lv_list_set_style(ui->list_2, LV_LIST_STYLE_BTN_REL, &list_style_rel);
	lv_list_set_style(ui->list_2, LV_LIST_STYLE_BTN_PR, &list_style_pr);
	lv_list_set_style(ui->list_2, LV_LIST_STYLE_BTN_TGL_REL, &list_style_tgl_rel);
	lv_list_set_style(ui->list_2, LV_LIST_STYLE_BTN_TGL_PR, &list_style_tgl_pr);
}
#endif
...
static int create_explorer(void)
{
	...
	lv_cont_set_style(Explorer_Para.ui.cont, LV_CONT_STYLE_MAIN,
			  &cont_style);
	...
	lv_obj_align(back_btn, Explorer_Para.ui.cont, LV_ALIGN_IN_TOP_LEFT, 0,
		     0);
	...
	int index = EXPLORER_IMAGE_AUDIO;
	for (; index < EXPLORER_IMAGE_NUM_MAX;index++) {
		get_image_buff_form_list(explorer_image_list,
							EXPLORER_IMAGE_NUM_MAX,
							index);
	}
	...
	lv_obj_set_event_cb(Explorer_Para.ui.image_1,
			    Explorer_Return_Home_Event);
...
#ifdef LV_USE_IMG
		Explorer_Para.backImageBtn =
		    lv_img_create(Explorer_Para.ui.cont, NULL);
		lv_obj_set_pos(Explorer_Para.backImageBtn, 763, 59);
		lv_obj_set_size(Explorer_Para.backImageBtn, 32, 32);
		lv_img_set_src(Explorer_Para.backImageBtn,
			       get_image_buff_form_list(explorer_image_list,
							EXPLORER_IMAGE_NUM_MAX,
							EXPLORER_BACK_IMAGE));
		lv_obj_set_click(Explorer_Para.backImageBtn, true);
		lv_obj_set_ext_click_area(Explorer_Para.backImageBtn, 27, 27,
					  27, 27);
		lv_obj_set_event_cb(Explorer_Para.backImageBtn,
				    Explorer_Back_Btn_Event);
#endif // LV_USE_IMG
	}
...
static page_interface_t page_explorer = {
    .ops =
	{
	    create_explorer, destory_explorer, show_explorer, hide_explorer,
	    msg_proc_explorer,
	},
    .info = {.id = PAGE_EXPLORER, .user_data = NULL}};
