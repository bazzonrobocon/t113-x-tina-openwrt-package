...
int ota_verify_file(char *file_path)
{
	...
	sprintf(command, "swupdate -i %s -c -n", file_path);
	printf("check command = %s",command);
	stream = popen(command, "r");
	if(!stream) {
		return -1;
	}
...
int ota_check_file(char *file_path) {
	...
	printf("ota check file error\n");
	return -1;
}
static void ota_disk_hotplug_callback(hotplug_disk_message_t *hotplug_message) {
	if (hotplug_message->operate == MEDIUM_PLUGOUT && strncmp(ota_firmware_path, hotplug_message->diskinfo->MountPoint, strlen(hotplug_message->diskinfo->MountPoint)) == 0)
	{
...
//	sprintf(command, "fw_setenv swu_mode upgrade_kernel");
//	system(command);

//	sprintf(command, "fw_setenv boot_partition recovery");
//	system(command);

	sprintf(command, "nice -n 19 swupdate_cmd.sh -i %s -e stable,upgrade_recovery &", ota_firmware_path);
	system(command);
	//system("reboot");
	return 0;
}
