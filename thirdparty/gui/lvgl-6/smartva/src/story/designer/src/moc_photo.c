...
static void btn_info_event(lv_obj_t * btn, lv_event_t event)
{
	if (event == LV_EVENT_CLICKED) {
		...
		get_file_size_info(filename, str, sizeof(str));
		lv_label_set_text(image_mgr.ui.label_file_size, str);

		memset(str, 0x00, sizeof(str));
		get_file_time_info(filename, str, sizeof(str));
...
static void image_list_event(lv_obj_t * btn, lv_event_t event)
{
	...
	if (event == LV_EVENT_CLICKED) {
		...
		image_show(image_mgr.file_path, IMAGE_SHOW_MODE_THUMB);
...
static void image_preview_full_screen_event(lv_obj_t * btn, lv_event_t event)
{
	if (event == LV_EVENT_CLICKED) {
		...
		image_show(image_mgr.file_path, IMAGE_SHOW_MODE_NORMAL);
		image_disp_cache_enable(0);
		usleep(20000);
		button_setting_interface_ui_hide(1);
//		image_button_redraw();
...
static int is_gif_file(char const *filename)
{
	...
	while(1)
	{
		if(image_play_get_status() == IMAGE_PLAY_START)
		{
			...
			while (1)
			{
				...
				if(is_gif == 1)
				{
					if(gif_play_is_finish() == 1 && delay_cnt == 0)
					{
						printf("we get finish\n");
						break;
					}
				}
				else
		...
		if(image_play_get_status() == IMAGE_PLAY_START)
		{
			...
			if(ret == 0 && is_gif_file(image_mgr.file_path) == 1)
			{
				is_gif = 1;
				gif_play_start(image_mgr.file_path,GIF_MODE_CIRCLE);
			}
			else
			{
				is_gif = 0;
...
static int image_switch(image_dir_t dir)
{
	...
	ret = image_show(image_mgr.file_path, IMAGE_SHOW_MODE_NORMAL);
	image_disp_cache_enable(0);
...
static void dialog_task(lv_task_t *ptask)
{
	photo_hide_dialog();
	printf("photo end1!\n");
	if(current_page() == PAGE_PHOTO)
	{
		switch_page(PAGE_PHOTO, PAGE_HOME);
	}
...
static void photo_show_dialog(const char* message, bool is_btn ,int time)
{
	...
	if(time > 0)
	{
		task  = lv_task_create(dialog_task, time*1000, LV_TASK_PRIO_MID, NULL);
		lv_task_once(task);
	}
}
...
static int font_change(void)
{
	...
	media_list_style_rel.text.color = lv_color_hex(0xffffff);
	media_list_style_rel.text.font = get_font_lib()->msyh_20;
	media_list_style_pr.text.color = lv_color_hex(0xffffff);
	media_list_style_pr.text.font = get_font_lib()->msyh_20;
	media_list_style_tgl_rel.text.color = lv_color_hex(0xffffff);
	media_list_style_tgl_rel.text.font = get_font_lib()->msyh_20;
	media_list_style_tgl_pr.text.color = lv_color_hex(0xffffff);
...
//	lv_cont_set_layout(image_mgr.ui.button_setting_interface, LV_LAYOUT_ROW_M);
	...
	else
	{
		...
		if(strlen(image_mgr.app_data.filename) != 0)
		{
			image_mgr.file_path  = image_mgr.app_data.filename;
			image_mgr.file_index = rat_npl_file2index(media_file_list->media_hrat,image_mgr.file_path);
			if(image_mgr.file_index == -1)
			{
		...
		media_set_list_focus(image_mgr.ui.media_list,image_mgr.file_index);
	}
	if(user_data)
	{
	...
	ret = pthread_create(&image_mgr.auto_play_id, &attr, image_auto_play_pthread, NULL);
	if(ret < 0)
	{
...
static int destory_photo(void)
{
	...
	pthread_mutex_destroy(&image_busy_mutex);
	pthread_cond_destroy(&image_wait_cond);
	image_exit();
	media_unload_file(media_file_list);
	image_mgr.app_data.effect = image_mgr.effect;
	image_mgr.app_data.speed  = image_mgr.speed;
	strcpy(image_mgr.app_data.filename,image_mgr.file_path);
	photo_write_breaktag(&image_mgr.app_data);
	image_mgr.is_init = 0;
	return 0;
...
{
	if(msg && (msg->type == MSG_SDCARD || msg->type == MSG_USB))
	{
		if(msg->value == 1)//plugout
		{
			destory_photo();
		}
	}
