...
static hotplug_disk_message_t Disk_HotPlugMsg;
static hotplug_disk_message_t Disk_HotPlugTf;
...
static void sound_disk_manager_callback(hotplug_disk_message_t *hotplug_message)
{
	player_ui_t *player_ui = media_get_player_data();

	if(hotplug_message->diskinfo->MediaType == MEDIUM_USB_MASSSTORAGE){
		memset(&Disk_HotPlugMsg, 0x00, sizeof(hotplug_disk_message_t));
		memcpy(&Disk_HotPlugMsg, hotplug_message, sizeof(hotplug_disk_message_t));
	}
	if(hotplug_message->diskinfo->MediaType == MEDIUM_SD_CARD){
		memset(&Disk_HotPlugTf, 0x00, sizeof(hotplug_disk_message_t));
		memcpy(&Disk_HotPlugTf, hotplug_message, sizeof(hotplug_disk_message_t));
	}
	switch(hotplug_message->operate){
		case MEDIUM_PLUGOUT:
			if(!record_disk){
				com_err("no disk!!!");
				goto end;
			}
			if(hotplug_message->diskinfo->MediaType == record_disk->MediaType){
				if(hotplug_message->diskinfo->MediaType == MEDIUM_USB_MASSSTORAGE && Disk_HotPlugTf.operate == MEDIUM_PLUGIN){
					record_disk = Disk_HotPlugTf.diskinfo;
				}else if(hotplug_message->diskinfo->MediaType == MEDIUM_SD_CARD && Disk_HotPlugMsg.operate == MEDIUM_PLUGIN){
					record_disk = Disk_HotPlugMsg.diskinfo;
				}else{
					record_disk = NULL;
					goto end;
				}
...
static int create_sound(void)
{
	...
	Disk_HotPlugMsg.diskinfo= NULL;
	Disk_HotPlugMsg.operate = MEDIUM_PLUGOUT;
	Disk_HotPlugTf.diskinfo= NULL;
	...
	media_func_register(EXT_MUSIC_SCENE, media_sound_ui_callback, NULL);
	lv_bar_set_value(para.ui.volume_bar, player_ui->media_cfg.volume, LV_ANIM_OFF);
	media_ui_send_event(MEDIA_UPDATE_LIST_EVENT, NULL, 0);
