...
static __s32 Explorer_Add_FileListItem(rat_media_type_t type, const char *FileName){
	...
	switch(type) {
		...
		case RAT_MEDIA_TYPE_FOLDER:
			...
			lv_obj_align(img_more,img_type,LV_ALIGN_OUT_RIGHT_MID,510,0);//???????????????????
...
static void Explorer_FileList_SwitchToApp(int Index) {
	...
	if (media_type == RAT_MEDIA_TYPE_FIRMWARE) {
...
static void Explorer_MediumList_AddItem(unsigned int FocusItem, unsigned int Index) {
	...
	char *string = NULL;
#endif
	file_item = get_file_list_item(Explorer_Para.explorer_list->top_file_list, Index);//??node?????????
	type = get_file_list_item_file_type(file_item);
	switch(type) {
...
#else
	btn_tmp = lv_list_add_btn(Explorer_Para.ui.medium_list_1, img_src, string);
#endif
	if (FocusItem == Index) {
...
//	rat_media_type_t Type = RAT_MEDIA_TYPE_EBOOK;
	...
	{
		int index = 0;
		for (index = 0; index < sizeof(explorer_image_list)/sizeof(explorer_image_list[0]); index++) {
			get_image_buff_form_list(explorer_image_list, EXPLORER_IMAGE_NUM_MAX, index);
		}
	}
