...
int ota_verify_file(char *file_path)
{
	...
	sprintf(command, "swupdate -i %s -k /etc/swupdate_public.pem -n ", file_path);
	printf("%s %d %s command:%s\n", __FILE__, __LINE__, __func__, command);
	stream = popen(command, "r");
	if(!stream) {
		printf("popen error ota verify file failed");
		return -1;
	}
	while (fgets(command, sizeof(command), stream)) {
		if (strstr(command, "successfully") != NULL) {
			pclose(stream);
			return 0;
		}
	}
	pclose(stream);
	printf("ota verify file failed");
    return -1;
}
...
int ota_check_file(char *file_path) {
	if (ota_verify_file(file_path) == 0) {
		...
		awjd("set_ota_cmd = %s ", set_ota_cmd);
		system(set_ota_cmd);
#endif
		return 0;
	}
...
static int create_ota(void)
{
	...
	ota_auto_ui_create(&para.ui);
	sprintf(command, "swupdate -i %s -e stable,%s -k /etc/swupdate_public.pem -v &", getenv("swu_software"), getenv("swu_mode"));
	printf("%s %d %s command:%s\n", __FILE__, __LINE__, __func__, command);
	system(command);
